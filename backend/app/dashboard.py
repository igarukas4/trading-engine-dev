...
class DashboardHub:
    ...
    def publish(
        self, stream: str, account_id: str | None, event_type: str,
        payload: dict[str, Any], *, aggregate_id: str | None = None,
        aggregate_version: int = 1, correlation_id: str | None = None,
    ) -> dict[str, Any]:
        if stream not in {"account", "system"}:
            raise ValueError("stream must be account or system")
        if stream == "account" and not account_id:
            raise ValueError("account stream requires broker_account_id")
        if stream == "system" and account_id is not None:
    ...
    def connect(
        self, cursors: dict[str, Any], *, account_ids: set[str] | None = None,
    ) -> dict[str, Any]:
        ...
        for key, cursor in [("system", system_cursor)]:
            events.extend(self._replay(key, None, cursor))
        for account_id, cursor in account_cursors.items():
            if account_ids is not None and account_id not in account_ids:
                events.append(self._snapshot_required("account", account_id, "UNAUTHORIZED_CURSOR"))
            else:
                events.extend(self._replay("account", account_id, cursor))
        events.sort(key=lambda event: (event["stream"] != "system", event.get("stream_sequence", 0)))
        return {"events": events, "connected_at": _now()}

    def _replay(self, stream: str, account_id: str | None, cursor: Any) -> list[dict[str, Any]]:
        key = (stream, account_id)
        try:
        ...
        replay.append({
            "type": "replay.complete", "stream": stream,
            "broker_account_id": account_id, "stream_sequence": latest,
        })
        return replay

    @staticmethod
    def _snapshot_required(stream: str, account_id: str | None, reason: str) -> dict[str, Any]:
        return {
            "type": "snapshot.required", "stream": stream,
            "broker_account_id": account_id, "reason": reason,
        }
...
class AuditHub:
    ...
    def list(self, account_id: str, limit: int = 50, cursor: int = 0) -> dict[str, Any]:
        with self._lock:
            items = list(reversed(self.events[account_id]))
        page = items[cursor:cursor + limit]
        return {"account_id": account_id, "audit_events": page,
