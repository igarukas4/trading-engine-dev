...
failure_service_file="$repository_root/deploy/systemd/trading-engine-backup-failure@.service"
...
[[ -f "$failure_service_file" ]] || fail 'scheduled backup failure alert service must exist'
...
grep -Fq 'OnFailure=trading-engine-backup-failure@%n.service' "$service_file" || fail 'scheduled backup must trigger an alert on failure'
grep -Fq 'telegram-alert.sh' "$failure_service_file" || fail 'failure alert service must invoke the Telegram sender'
