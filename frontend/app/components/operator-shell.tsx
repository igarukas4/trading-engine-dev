...
export function OperatorShell({ children }: { children: ReactNode }) {
  ...
  function chooseAccount(value: string) {
    setSelected(value);
    if (value) {
      window.localStorage.setItem("selected-broker-account", value);
    } else {
      window.localStorage.removeItem("selected-broker-account");
    }
  }
  ...
  async function emergency(kind: "STOP_ONLY" | "CLOSE_ALL") {
    ...
    const response = await fetch(`${apiBase}/api/v1/global-emergency`, {
      ...
      body: JSON.stringify({
        ...
        kind,
        reason: "Tindakan darurat operator",
      }),
    });
    setMessage(
      response.ok
        ? "Diproses — status final mengikuti operasi darurat."
        : "Perintah darurat ditolak oleh backend.",
    );
  ...
  return (
    <div>
      <header aria-label="Konteks operator">
        <Link href="/">Trading Engine</Link>
        <label>
          Akun
          <select
          ...
          >
            <option value="">Ringkasan Semua Akun</option>
        ...
        {selected && (
          <span data-testid="account-context">
        ...
        {emergencyOpen && (
          <aside role="dialog" aria-label="Konfirmasi darurat">
            <p>Konfirmasi scope akun dan dampak close-all.</p>
            <button type="button" onClick={() => emergency("STOP_ONLY")}>
              Stop exposure
            </button>
            <button type="button" onClick={() => emergency("CLOSE_ALL")}>
              Stop + close all
            </button>
      ...
      <nav aria-label="Navigasi operator">
        {links.map(([label, href]) => (
          <Link key={label} href={`${href}${query}`}>
            {label}
          </Link>
        ))}
      </nav>
