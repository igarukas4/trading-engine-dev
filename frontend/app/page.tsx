...
export default function DashboardPage() {
  ...
  const accounts = summary?.accounts ?? []; const positions = accounts.reduce((total, account) => total + account.open_positions, 0); const criticalAlerts = (summary?.critical_alerts ?? []).filter((alert, index, alerts) => alerts.findIndex(candidate => candidate.id === alert.id) === index);
