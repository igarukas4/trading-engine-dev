...
class ConnectorProtocol:
 ...
 def hello(self,secret):
  ...
  return {"type":"hello","account_id":self.cfg.account_id,"provider":self.cfg.provider,"broker_server":self.cfg.broker_server,"external_account_id":self.cfg.external_account_id,"key_id":self.cfg.key_id,"secret":secret,"generation":self.cfg.backend_generation,"session_id":self.session_id}
 def accept_snapshot(self,msg):
  ...
  incoming=msg.get("generation", self.cfg.backend_generation)
  if self.generation is None: self.generation=incoming
  elif incoming != self.generation: raise ProtocolError("generation mismatch")
  return msg["snapshot"]
 ...
 def backoff(self,attempt): return min(self.cfg.reconnect_max, max(0.0, self.cfg.reconnect_initial*(2**attempt)*(1+self.cfg.jitter*(self.random()*2-1))))
